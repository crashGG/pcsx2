...
uint ps_convert_float32_32bits(PS_INPUT input) : SV_Target0
{
	...
	return uint(min(exp2(32.0f) * sample_c(input.t).r, 4294967295.0f));
}
...
PS_OUTPUT ps_convert_float32_rgba8(PS_INPUT input)
{
	...
	uint d = uint(min(sample_c(input.t).r * exp2(32.0f), 4294967295.0f));
...
float ps_convert_float32_float24(PS_INPUT input) : SV_Depth
{
	...
	uint d = uint(min(sample_c(input.t).r * exp2(32.0f), 4294967295.0f)) & 0xFFFFFFu;
